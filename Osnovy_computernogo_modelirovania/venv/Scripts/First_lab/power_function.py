...
def get_value_s(n, points):
    ...
    current_sum = 0
    for i in range(n):
        current_sum += pow(beta*pow(points[0][i], a) - points[1][i], 2)
...
def form_polinom(a, beta):
    def polinom(list_x):
        list_y = list()
        for x in list_x:
            list_y.append(round(beta*pow(x,a),4))
        return list_y

    return polinom


def power_function_method(points):
